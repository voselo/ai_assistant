...
import (
	"ai_assistant/internal/model/customer"
...
func NewCustomerRepository(db *sqlx.DB) *CustomersRepository {
...
func (repository *CustomersRepository) Update(id string, updateDTO *customer.UpdateDTO) (*model.CustomerModel, error) {
...
func (repository *CustomersRepository) ValidateLicense(hash string) (uid string, err error) {
	...
	if customer.LicenseStatus == model.LicenseStatusActive && (customer.LicenseExpiresDate == nil || customer.LicenseExpiresDate.After(time.Now())) {
		...
		return "", errors.New("license not valid")
	}
